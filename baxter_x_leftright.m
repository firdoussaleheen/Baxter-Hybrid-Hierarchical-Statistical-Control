%QPLOT Plot Baxter robot end-effector position
%
...
function baxter_x_leftright(t, q)
    if nargin < 2
        q = t;
        t = (1:numrows(q))';
    end
...
q = 1000*q; % convert into mm
% square plot
figure('Position',[638   492   692   385]);hold on;
...
%     set(hData4, 'LineStyle', '-', 'Color',  'k', 'LineWidth', 2);
...
%     %hTitle = title('');
    ...
    hYLabel = ylabel('End-effector Position x-axis (mm)');
%     
    
    % Add legend
    hLegend = legend([hData1, hData2], ...
        'Agent 1 (right arm)', 'Agent 2 (left arm)');
%     
%     % Adjust font
